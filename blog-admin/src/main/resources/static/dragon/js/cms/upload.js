    ...
    $("#multipleFile").fileinput({
        ...
        if(code == 0){
            ...
            $.each(errorData,function(index,value){
                let fileName = value.fileName;
                let $fileName = $("div[title='"+fileName+"']");
                let fileUploadIndicator = $fileName.siblings(".file-upload-indicator");
                fileUploadIndicator.html('');
                fileUploadIndicator.html('<i class="glyphicon glyphicon-remove-sign text-danger"></i>');
            });
            ...
            let fileUploadIndicator = $(".file-upload-indicator");
            fileUploadIndicator.html('');
            fileUploadIndicator.html('<i class="glyphicon glyphicon-remove-sign text-danger"></i>');
            $.modal.alertError("文件上传失败！");
        }
